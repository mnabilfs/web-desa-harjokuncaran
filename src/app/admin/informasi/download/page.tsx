...
import { Save, X, Loader2, CheckCircle2, Plus, Trash2, Search, Edit2, FileText, Download } from "lucide-react";
...
export default function DownloadAdmin() {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    let query = supabase.from('download').select('*').order('created_at', { ascending: false });
  ...
  const handleDelete = async (id: string, fileUrl: string, itemTitle: string) => {
    if(!window.confirm(`Yakin ingin menghapus dokumen "${itemTitle}"?`)) return;
    
    setLoading(true);
    // 1. Delete from database
    await supabase.from('download').delete().eq('id', id);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (file) {
      ...
      const fileName = `doc_${Math.random().toString(36).substring(2, 10)}_${Date.now()}.${fileExt.toLowerCase()}`;
      ...
      if (uploadError) {
        alert("Gagal mengunggah dokumen: " + uploadError.message);
        setSaving(false);
        return;
      }
    ...
    let error = null;
    if (editingId) {
      const res = await supabase.from('download').update(payload).eq('id', editingId);
      error = res.error;
    } else {
      const res = await supabase.from('download').insert([payload]);
      error = res.error;
    }

    if (error) {
      alert("Gagal menyimpan data dokumen: " + error.message);
    } else {
      setSuccessMsg(`Dokumen berhasil ${editingId ? "diperbarui" : "ditambahkan"}!`);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-16">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold text-gray-800">Manajemen Pusat Unduhan</h1>
        <button 
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 text-center border border-gray-200 rounded-xl shadow-sm text-gray-500 flex flex-col items-center">
          ...
          {searchKeyword ? "Tidak ada dokumen yang cocok dengan pencarian." : "Belum ada dokumen. Silakan klik tombol \"Tambah Dokumen\"."}
        </div>
      ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
              <h2 className="text-xl font-bold text-gray-800">
                {editingId ? "Edit Dokumen" : "Unggah Dokumen Baru"}
              </h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              <div>
                ...
                <input required type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-black focus:ring-[#0088cc] focus:border-[#0088cc]" placeholder="Contoh: Peraturan Desa No. 1" />
              </div>
